# Shadowrocket屏蔽Adobe的网络验证
> ⚠️ shadowrocket开启后，全部流量会走VPN，原防火墙和hosts设置的屏蔽会失效！需在VPN设置里加上Adobe的屏蔽名单。

个人使用，遇到屏蔽失效时会更新。最后更新时间：2026-03-26

如有建议，可以在Issues中提交。

## 使用方法

1. 复制以下规则链接，在 小火箭 -> 配置 -> 远程文件 中添加
2. 小火箭 -> 配置 -> 模块，右上角，添加规则（模块化规则链接）
3. 完成

### 规则下载：
https://raw.githubusercontent.com/istraw/Shadowrocket_Adobe_Rule/refs/heads/main/Adobe_Rule.module


## 非VPN用户可以通过hosts等方式屏蔽
#### ignaciocastro的项目：
[a-dove-is-dumb](https://github.com/ignaciocastro/a-dove-is-dumb)
